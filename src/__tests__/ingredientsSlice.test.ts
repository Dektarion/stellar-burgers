...
const expectedIngredientsResult = [
  {
...
describe('test ingredientsSlice async actions', () => {
  ...
  it('should set error and loading=false on rejected', () => {
    ...
    const state = ingredientsSliceReducer(
      ...
      getIngredients.rejected(new Error(error), '', undefined)
    );
  ...
  it('should store data and set loading=false on fulfilled', () => {
    const state = ingredientsSliceReducer(
      ...
      getIngredients.fulfilled(expectedIngredientsResult, '', undefined)
    );

    expect(state.loading).toBe(false);
    expect(state.ingredients).toEqual(expectedIngredientsResult);
  });
});
